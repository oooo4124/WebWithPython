...
function danprice(msg) {
	...
	var CPM = [ "노출예상", 2500, 5000, 7500, 10000, 12500, 15000, 20000];
	var CPV = [ "노출예상(조건충족)", 1000, 2000, 3000, 5000, 7000, 10000, 20000 ];
...
function appendRs2(form){
	...
	var temptemp = replaceAll(rs.innerHTML," ","_");
	$("#final").append("<input type='hidden' id="+cnt+" name=finalprice value="+parseInt(rs2.innerHTML)+">");
	//alert(rs2.innerHTML);
	...
	$("#final").append("<input type='hidden' id="+cnt+" name=final value='<네이버>_"+temptemp+"'>");
...
function appendRs5(form){
	...
	var temptemp = replaceAll(rs.innerHTML+" "+rs2.innerHTML+" "+rs3.innerHTML+" "+rs4.innerHTML," ","_");
	...
	$("#final").append("<input type='hidden' id="+cnt+" name=final value='<네이버>_"+temptemp+"'>");
	$("#final").append("<input type='hidden' id="+cnt+" name=finalprice value="+parseInt(rsTemp[1])+">");
	
	
	$("#finalSum").html("총 예상견적 : "+ sumTemp + "원");
}

function appendRs5Youtube(form){
	Name = form.name;
	var rs = document.getElementById(Name+"Rs");
	var rs2 = document.getElementById(Name+"Rs2");
	var rs3 = document.getElementById(Name+"Rs3");
	...
	$("#final").append("<span id="+cnt+">"+rs.innerHTML+" "+rs2.innerHTML+" "+rs3.innerHTML+" "+rs4.innerHTML+" "+rs5.innerHTML
			+"<input type='button' name = "+rsTemp[1]+" id="+cnt+" value='취소' onclick= del(this.id,this.name)>"+"<br>"+"</span>");
	$("#final").append("<input type='hidden' id="+cnt+" name=final value='<유튜브>_"+temptemp+"'>");
	$("#final").append("<input type='hidden' id="+cnt+" name=finalprice value="+parseInt(rsTemp[1])+">");
...
function appendDaumRs5(form){
	...
	$("#final").append("<input type='hidden' id="+cnt+" name=final value='<다음>_"+temptemp+"'>");
	$("#final").append("<input type='hidden' id="+cnt+" name=finalprice value="+parseInt(rs5.innerHTML)+">");
	$("#finalSum").html("총 예상견적 : "+ sumTemp + "원");
}
function appendNaverHomeRs(form){
	...
	$("#final").append("<span id="+cnt+">네이버홈 통합 스페셜 "+rs5.innerHTML
			+"<input type='button' name = "+parseInt(rs5.innerHTML)+" id="+cnt+" value='취소' onclick= del(this.id,this.name)>"+"<br>"+"</span>");
	$("#final").append("<input type='hidden' id="+cnt+" name=final value='<네이버>_네이버홈_통합_스페셜'>");
	$("#final").append("<input type='hidden' id="+cnt+" name=finalprice value="+parseInt(rs5.innerHTML)+">");
	$("#finalSum").html("총 예상견적 : "+ sumTemp + "원");
}
...
function insertFinal(form,id){
	...
	var temptemp = replaceAll(site.innerHTML," ","_");
	$("#final").append("<input type='hidden' id=del"+cnt+" name=finalprice value="+priceTTemp+">");
	$("#final").append("<input type='hidden' id=del"+cnt+" name=final value='<"+Name+">_"+temptemp+"'>"
			);
...
function openPOP(form){
	...
	if(sumTemp != 0){
		window.open("", "popup_window", "width=645, height=700, scrollbars=yes");
	    $("#myform").submit();
	}else{
		alert("최소 한개이상의 상품을 선택해주세요.")
	}
...
<noscript>
	<link rel="stylesheet"
		href="html5up-hyperspace/assets/css/noscript.css" />
</noscript>
</head>
	...
	<header id="header">
		...
		<nav>
			<ul>
				...
				<li><a href="html5up-hyperspace/Online.jsp" class="active">온라인
						분석</a></li>
			...
			<div class="inner">
				...
				<div id="printDiv">
					<%
						...
						//}
						String 대분류 = request.getParameter("대분류");
					...
					<br><br>
					<%System.out.println("test!! : "+소분류);
					if(!소분류.equals("-") && 나이 != null && 목적 !=null && 타입 != null){ %>
					<h1 class="major">온라인 광고 분석</h1>
					
					<img src ='html5up-hyperspace/images/logos/keyword.png' width=140px height=130px>
					<h2>선택한 주제에 해당하는 키워드에 대한 정보</h2>
					<table>
						<tr>
							<td><b>키워드</b></td>
							<td><b>주성별(M: 남 F: 여 S: 공통)</b></td>
							<td><b>1순위연령대</b></td>
							<td><b>2순위연령대</b></td>
							<td><b>최대유입기간(월)</b></td>
							<td><b>최소유입기간(월)</b></td>
						...
						<%
								...
								for (int i = 0; i < rvo.get(2).size(); i++) {
									String nameTemp = rvo.get(2).get(i).get키워드();
									System.out.println("test: "+nameTemp);
									String srcTemp = "./wordcloude/"+adread.keyword(nameTemp)+".PNG";
									System.out.println("test: "+srcTemp);
									
									
						%>
						<tr>
							<td><a href=<%=srcTemp%> target="_blank"><b><%=nameTemp%></b></a></td>
					...
					<%if(타입!=null && 성별 != null && 나이 != null){ %>
					<img src ='html5up-hyperspace/images/logos/score.png' width=150px height=100px>
					...
					<table>
						...
						%>
						<tr>
							<td><b><%=rvo.get(4).get(i).get타겟()%></b></td>
							<%
					...
					<%if(목적!=null && 나이 != null){ %>
					<img src ='html5up-hyperspace/images/logos/top.png' width=130px height=140px>
					<h2>추천하는 광고 기법</h2>
					<table>
						<%
							for (int i = 0; i < 목적.length; i++) {
						%>
						<tr>
							<td colspan="5"><b><%=rvo.get(3).get(i).get목적()%></b> 목적에 따른 적합한
								광고기법 순위(%)</td>
						</tr>
						<tr>
							<td><b>1순위</b></td>
					...
					<br><br>
					<h1 class="major">예상 견적 계산</h1>
					<%
					if(!소분류.equals("-") && 목적 != null && 나이 != null){ %>
					<img src ='html5up-hyperspace/images/logos/blog.png' width=120px height=120px>
					<h2>추천 블로그</h2>
					<form name=블로그광고>
						<table>
							<tr>
							...
							<tr>
								...
								<td><a id=블로그광고이름bl<%=i + 1%>
									href="<%=rvo.get(0).get(i).get링크()%>"><b><%=rvo.get(0).get(i).get링크()%></b></a></td>
					...
					<img src ='html5up-hyperspace/images/logos/cafe.png' width=120px height=120px>
					<h2>추천 카페</h2>
					...
					<form name=카페광고>
						<table>
							...
							<tr>
								<td><b><%=i + 1%></b></td>
								<td id=카페광고이름ca<%=i + 1%>><b><%=rvo.get(1).get(i).get이름()%></b></td>
					...
					<img src ='html5up-hyperspace/images/logos/naverL.png' width=240px height=50px>
					<h2>네이버</h2>
					<h4>네이버 PC 브랜드검색</h4>
					<form name="NaverPcBrandSearch">
						...
						</span> <input type=button value=선택 onclick=appendRs2(this.form)>
							<span id="NaverPcBrandSearchRs"></span> <span
						id="NaverPcBrandSearchRs2"></span></form>
					...
					<form name="NaverMBrandSearch">
						...
						id="NaverMBrandSearchRs2"></span></form>
					...
					<form name="NaverMContentDA">
						<select name="sel1" onchange="sub(this.form);">
							...
							<option value="">선택하세요</option>
						</select> <input type=button value=선택 onclick=appendRs5(this.form)>
						<span id="NaverMContentDARs"></span> <span
							id="NaverMContentDARs2"></span> <span id="NaverMContentDARs3"></span>
						<span id="NaverMContentDARs4"></span> <span
					...
					<form name="NaverSubVideo">
						...
						<span id="NaverSubVideoRs"></span> <span
					...
					<span id="NaverTimeBoardRs"></span> <span id="NaverTimeBoardRs2"></span></form>
					...
					<form name="NaverHomeRol">
						...
						<span id="NaverHomeRolRs"></span> <span
							id="NaverHomeRolRs2"></span> <span id="NaverHomeRolRs3"
							value=hello></span> <span id="NaverHomeRolRs4"></span> <span
							id="NaverHomeRolRs5"></span>
					...
					<img src ='html5up-hyperspace/images/logos/daum.png' width=205px height=80px>
					<h2>다음</h2>
					<h4>다음 PC 브랜드검색</h4>
					<form name="DaumPcBrandSearch">
						...
						</span> <input type=button value=선택 onclick=appendDaumRs5(this.form)>
						<span id="DaumPcBrandSearchRs"></span> <span
					...
					<img src ='html5up-hyperspace/images/logos/youtube.png' width=250px height=90px>
					<h2>유튜브</h2>
					<h4>유튜브 영상 광고</h4>
					<form name="Youtube">
						<select name="sel1" onchange="sub(this.form);">
							...
							<option value="">선택하세요</option>
						</select> <input type=button value=선택 onclick=appendRs5Youtube(this.form) >
						<span id="YoutubeRs"></span> <span id="YoutubeRs2"></span>
						<span id="YoutubeRs3" value=hello></span> <span id="YoutubeRs4"></span>
						<span id="YoutubeRs5"></span>

					</form>
					</div>
					<form id="myform" name="myform" action=popup2.jsp
					 target="popup_window" method = post>

						<span id="final" ></span> <br> <span id="finalSum">총
							예상견적 : 0원</span> <br> <br> <br> <br>
							
							
				<p>
					
					<input type="button" id = btn_submit onclick = 'openPOP(this.form)' value='예상 견적서 보기' >
				</p>
